...
import penerbit from '@/assets/penerbit.json';
...
const paginatedData = computed(() => {
    ...
    const filteredItems = items.value.filter(item => item.nama.toLowerCase().includes(store.searchQuery.toLowerCase()));
    ...
    <div>
        <ul class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 container mx-auto gap-5 px-6">
            <li v-for="(item, index) in paginatedData" :key="index">
                <router-link to="/single" class="flex gap-4 max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-blue-50 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                   <div class="w-14 h-14 rounded-full bg-neutral-100 shrink-0 flex items-center justify-center">Logo</div>
                    <div>
                    <h5 class="mb-2 font-medium tracking-tight text-blue-700 dark:text-white leading-none">{{ excerptTitle(item.nama) }}</h5>
                    <p class="text-sm flex-grow font-normal text-gray-700 dark:text-gray-400 ">No Anggota: {{ item.no_nggota }}</p>
                   </div>
